...
# 画面全体のデザイン設定
st.set_page_config(page_title="菅川橋 水位予測システム V3.2", page_icon="🌊", layout="wide")

st.title("🌊 菅川橋 水位予測システム (ハイブリッド・強気チューニング版)")
st.markdown("現況水位と雨量のバランスを最適化し、**過小評価（引き波バイアス）も過大評価（暴走）も防ぐようにチューニングされた**最新システムです。")
...
if uploaded_file is not None:
    try:
        ...
        fig.add_trace(go.Scatter(x=pred_times, y=pred_wl_list, name='AI基本予測 (V3新モデル)', mode='markers+lines', line=dict(color='orange', dash='dash')))
